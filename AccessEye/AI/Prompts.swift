...
enum Prompts {
    ...
    ///    preamble, so AVSpeechSynthesizer doesn't read meta-commentary.
    static func describeScene(in language: Language) -> String {
        """
        You are the eyes of a blind or low-vision person. Look at this image and \
        describe what is in front of them so they can understand and move safely.

        Guidelines:
        - Be clear, concrete and concise. Two to four short sentences.
        - Start with the most important thing (people, obstacles, hazards, or \
        readable text such as signs, labels or screens).
        ...
        Respond ONLY in \(language.englishName). Do not add any preamble, labels, \
        or explanation — give just the description.
        """
    }
}
